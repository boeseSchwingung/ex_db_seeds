defmodule ExDbSeeds.Seed.Runner do
  ...
  defp handle_run_seed({time, {:ok, artifacts}}, opts) do
    ...
    Enum.each(updateds, fn {updated, changes} ->
      log(
        opts[:log],
        "Updated #{inspect(updated.__struct__)}{id: #{inspect(updated.id)}} with changes: #{inspect(changes)}"
      )
    end)
    ...
    Enum.each(errors, fn changeset ->
      log(
        opts[:log],
        "ERROR #{inspect(changeset.errors)} on #{changeset.model.__struct__}{id: #{inspect(changeset.model.id)}} with changes #{inspect(changeset.changes)}"
      )
    end)
